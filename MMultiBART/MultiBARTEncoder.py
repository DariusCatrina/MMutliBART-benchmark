...
class MultiBARTEncoder(BartEncoder):
    ...
    def __init__(self, config, embed_tokens: Optional[nn.Embedding] = None, bart_seq_len=1024):
        config.d_model = 768
    ...
    def forward(
        ...
        num_chunks = seq_len // self.bart_seq_len 
        #assert num_chunks == 1, (seq_len, num_chunks, MultiBARTEncoder.bart_seq_len) 

        # 1
        input_ids = torch.reshape(input_ids, (batch_size * num_chunks, self.bart_seq_len))
        attention_mask = torch.reshape(attention_mask, (batch_size * num_chunks, self.bart_seq_len))
        ...
        concat_embeddings = torch.reshape(encoder_outputs[0], (batch_size, num_chunks*self.bart_seq_len, 768)) # batch_size x seq_len x embedding dim
